...
#define PRELOG "[Visu::PointNormals] "
...
VuRenderObject *vu_point_normals_new(VuVisu *visu,
                                     pCloud points,
                                     pCloud normals,
                                     vec4 color,
                                     float point_size) {
    ...
    // public
    VuPointNormals *self = p_rhc_calloc(sizeof *self);
    ...
    object_base_init(&impl->super, visu, "PointNormals", &program, unload, kill_impl, update, render);
...
static void kill_impl(VuRenderObject *super) {
    VuPointNormals *self = (VuPointNormals *) super;
